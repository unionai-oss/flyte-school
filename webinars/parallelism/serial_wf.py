from union import task, workflow, dynamic, ImageSpec
import pandas as pd
...
@task(container_image=custom_image)
def train_models_for_hyperparameters(data: pd.DataFrame, hyperparameters: list[float]) -> list[float]:
    accuracies = []
...
def dynamic_workflow(hyperparameters: list[float]) -> list[float]:
    ...
    # Train models for each hyperparameter
    accuracies = train_models_for_hyperparameters(data=data, hyperparameters=hyperparameters)
...
@workflow
def serial_ml_pipeline() -> list[float]:
